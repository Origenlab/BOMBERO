...
const ALERTAS_QUINTANA_ROO = {
  ...
  crucerosMasivos: {
    activa: () => {
      const dia = (/* @__PURE__ */ new Date()).getDay();
      return dia >= 1 && dia <= 5;
    },
    mensaje: {
...
function getEstacionesBilingues() {
  return estacionesQuintanaRoo.filter(
    (e) => e.idiomas?.includes("Inglés") || e.caracteristicasEspeciales?.some((c) => c.toLowerCase().includes("bilingüe"))
  );
}
function getEstacionesRescateAcuatico() {
  return estacionesQuintanaRoo.filter(
    (e) => e.servicios?.some(
      (s) => s.toLowerCase().includes("acuático") || s.toLowerCase().includes("buceo") || s.toLowerCase().includes("marítimo")
    )
  );
}
function getEstacionesHoteleras() {
  return estacionesQuintanaRoo.filter(
    (e) => e.servicios?.some((s) => s.toLowerCase().includes("hotel")) || e.caracteristicasEspeciales?.some(
      (c) => c.toLowerCase().includes("resort") || c.toLowerCase().includes("hotel") || c.toLowerCase().includes("habitaciones")
    )
  );
}
function getEstacionesAeropuertos() {
  return estacionesQuintanaRoo.filter(
    (e) => e.servicios?.some(
      (s) => s.toLowerCase().includes("ssei") || s.toLowerCase().includes("aeronáutico") || s.toLowerCase().includes("aeronaves")
    )
  );
}
function getEstacionesCruceros() {
  return estacionesQuintanaRoo.filter(
    (e) => e.servicios?.some((s) => s.toLowerCase().includes("crucero")) || e.caracteristicasEspeciales?.some((c) => c.toLowerCase().includes("crucero"))
  );
}
function getEstacionesIslas() {
  return estacionesQuintanaRoo.filter(
    (e) => e.zona === "Cozumel" || e.zona === "Isla Mujeres"
  );
}
...
function getAlertasActivas(zona, idioma = "es") {
  ...
  for (const [, alerta] of Object.entries(ALERTAS_QUINTANA_ROO)) {
    if (alerta.activa()) {
      if (!zona || alerta.zonas.includes(zona) || alerta.zonas.includes("Todas")) {
        const mensaje = typeof alerta.mensaje === "object" ? alerta.mensaje[idioma] || alerta.mensaje.es : alerta.mensaje;
        alertasActivas.push({
          mensaje,
          tipo: alerta.tipo,
          icono: alerta.icono,
...
function getEstadisticasTurismo() {
  return INDUSTRIA_TURISTICA_QR.estadisticas;
}
function getZonasHoteleras() {
  return INDUSTRIA_TURISTICA_QR.zonasHoteleras;
}

export { FAQ_QUINTANA_ROO as F, QUINTANA_ROO_SEO as Q, ZONAS_QUINTANA_ROO as Z, getAlertasActivas as a, generarSchemaEstacion as b, getEstacionesBilingues as c, getEstacionesRescateAcuatico as d, estacionesQuintanaRoo as e, getEstacionesHoteleras as f, getEstacionesPorZona as g, getEstacionesAeropuertos as h, getEstacionesCruceros as i, getEstacionesIslas as j, getEstadisticasTurismo as k, getZonasHoteleras as l, generarInterlinking as m };
