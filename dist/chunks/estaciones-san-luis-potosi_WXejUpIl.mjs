...
function getEstacionesHAZMAT() {
  return estacionesSanLuisPotosi.filter(
    (e) => e.servicios?.some((s) => s.toLowerCase().includes("hazmat")) || e.caracteristicasEspeciales?.some((c) => c.toLowerCase().includes("hazmat"))
  );
}
function getEstacionesEspeleologia() {
  return estacionesSanLuisPotosi.filter(
    (e) => e.servicios?.some((s) => s.toLowerCase().includes("espeleolog")) || e.caracteristicasEspeciales?.some(
      (c) => c.toLowerCase().includes("sótano") || c.toLowerCase().includes("espeleolog")
    )
  );
}
function getEstacionesSwiftWater() {
  return estacionesSanLuisPotosi.filter(
    (e) => e.servicios?.some(
      (s) => s.toLowerCase().includes("swift water") || s.toLowerCase().includes("acuático")
    )
  );
}
...
function getEstadisticasIndustria() {
  return INDUSTRIA_AUTOMOTRIZ_SLP.resumen;
}
function getAtraccionesHuasteca() {
...
export { FAQ_SLP as F, INFRAESTRUCTURA_SLP as I, PROTOCOLOS_SLP as P, SAN_LUIS_POTOSI_SEO as S, ZONAS_SLP as Z, getAlertasActivas as a, generarSchemaEstacion as b, getPlantasAutomotrices as c, getEstacionesHAZMAT as d, estacionesSanLuisPotosi as e, getEstacionesEspeleologia as f, getEstacionesPorZona as g, getEstacionesSwiftWater as h, getEstacionesBilingues as i, getEstadisticasIndustria as j, getAtraccionesHuasteca as k, getPueblosMagicos as l, generarInterlinking as m };
