...
function getEstacionesHAZMAT() {
  return estacionesQueretaro.filter(
    (e) => e.servicios.some((s) => s.toLowerCase().includes("hazmat")) || e.caracteristicasEspeciales?.some((c) => c.toLowerCase().includes("hazmat"))
  );
}
function getEstacionesAeroespaciales() {
  return estacionesQueretaro.filter(
    (e) => e.zona === "Zona Industrial Norte" || e.caracteristicasEspeciales?.some(
      (c) => c.toLowerCase().includes("aeroespacial") || c.toLowerCase().includes("bombardier") || c.toLowerCase().includes("safran")
    )
  );
}
...
function getEstacionesUNESCO() {
  return estacionesQueretaro.filter(
    (e) => e.caracteristicasEspeciales?.some(
      (c) => c.toLowerCase().includes("unesco") || c.toLowerCase().includes("misiones")
    )
  );
}
function getEstacionesCarreteras() {
  return estacionesQueretaro.filter(
    (e) => e.servicios?.some(
      (s) => s.toLowerCase().includes("carretero") || s.toLowerCase().includes("autopista")
    ) || e.caracteristicasEspeciales?.some(
      (c) => c.toLowerCase().includes("autopista") || c.toLowerCase().includes("57d")
    )
  );
}
...
function getEmpresasAeroespaciales() {
  return INDUSTRIA_QUERETARO.aeroespacial.empresasPrincipales;
}
function getEstadisticasAeroespaciales() {
  return INDUSTRIA_QUERETARO.aeroespacial.estadisticas;
}

export { FAQ_QUERETARO as F, INDUSTRIA_QUERETARO as I, QUERETARO_SEO as Q, TURISMO_QUERETARO as T, ZONAS_QUERETARO as Z, getAlertasActivas as a, generarSchemaEstacion as b, generarInterlinking as c, getEstacionesHAZMAT as d, estacionesQueretaro as e, getEstacionesAeroespaciales as f, getEstacionesPorZona as g, getEstacionesPueblosMagicos as h, getEstacionesUNESCO as i, getEstacionesCarreteras as j, getEmpresasAeroespaciales as k, getEstadisticasAeroespaciales as l };
