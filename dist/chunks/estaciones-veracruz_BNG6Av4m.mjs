...
function getEstacionesMaritimas() {
  return estacionesVeracruz.filter(
    (e) => e.servicios?.some(
      (s) => s.toLowerCase().includes("marítimo") || s.toLowerCase().includes("puerto") || s.toLowerCase().includes("buque")
    )
  );
}
function getEstacionesPetroleras() {
  return estacionesVeracruz.filter(
    (e) => e.caracteristicasEspeciales?.some(
      (c) => c.toLowerCase().includes("pemex") || c.toLowerCase().includes("refinería") || c.toLowerCase().includes("petroquímic") || c.toLowerCase().includes("pozos")
    )
  );
}
