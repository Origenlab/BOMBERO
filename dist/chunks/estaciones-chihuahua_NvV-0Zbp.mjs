...
function getEstacionesCapital() {
  return estacionesChihuahua.filter(
    (e) => e.ciudad === "Chihuahua" && e.estado === "Chihuahua"
  );
}
function getEstacionesCiudadJuarez() {
  return estacionesChihuahua.filter(
    (e) => e.ciudad === "Ciudad Juárez"
  );
}
...
function getEstacionesForestales() {
  return estacionesChihuahua.filter(
    (e) => e.servicios.some(
      (s) => s.toLowerCase().includes("forestal") || s.toLowerCase().includes("montaña") || s.toLowerCase().includes("sierra")
    )
  );
}
function getEstacionesSierraTarahumara() {
  const ciudadesSierra = ["Creel", "Guachochi", "Bocoyna", "Madera"];
  return estacionesChihuahua.filter(
    (e) => ciudadesSierra.includes(e.ciudad)
  );
}
...
export { ESTADISTICAS_CHIHUAHUA as E, getEstacionesCercanas as a, estacionesChihuahua as e, getEstacionBySlug as g };
